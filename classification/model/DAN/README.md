...
5. **Testing**. Modify the `test_file_path`,`deploy_file_path` and `weight_path` in [exp_pred.m](exp_pred.m), and run `>> exp_pred()` in Matlab. The function will generate a text file containing all the  in `../../result/` with the most recent timestampe.
